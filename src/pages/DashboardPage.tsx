...
const EDIT_EXPIRATION_OPTIONS = [
  { label: "Sin cambios en el límite actual", value: -1 },
  {
    label: "Otorgar 3 días extra a partir de hoy",
    value: 3 * 24 * 60 * 60 * 1000,
  },
  {
    label: "Otorgar 5 días extra a partir de hoy",
    value: 5 * 24 * 60 * 60 * 1000,
  },
  {
    label: "Otorgar 7 días extra a partir de hoy",
    value: 7 * 24 * 60 * 60 * 1000,
  },
  {
    label: "Otorgar 15 días extra a partir de hoy",
    value: 15 * 24 * 60 * 60 * 1000,
  },
  {
    label: "Otorgar 30 días extra a partir de hoy",
    value: 30 * 24 * 60 * 60 * 1000,
  },
  { label: "Establecer Acceso Indefinido (Eterno)", value: 0 },
];

const DashboardPage: React.FC<{ initialTab?: "projects" | "users" }> = ({
  initialTab = "projects",
}) => {
  ...
  const [editDuration, setEditDuration] = useState<number>(
    EDIT_EXPIRATION_OPTIONS[0].value,
  );
  const [editRole, setEditRole] = useState<"admin" | "researcher">(
    "researcher",
  );
  ...
  const loadInitialData = async () => {
    ...
    try {
      if (activeTab === "projects") {
        const data = await projectService.getAllProjects(user!.uid, isAdmin);
        setProjects(data);
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    const expiresAt =
      newUser.duration === 0 ? null : Date.now() + newUser.duration;
  ...
  const handleSaveEditUser = async (e: React.FormEvent) => {
    ...
    if (editDuration > 0) {
      // Calculate renewal days from now
      expiresAt = Date.now() + editDuration;
    } else if (editDuration === 0) {
      expiresAt = null; // Eternal
    }
    // if editDuration === -1, we keep the existing expiresAt value
    ...
    try {
      await setDoc(doc(db, "users", editingUser.uid), userToUpdate);
      setEditingUser(null);
      loadInitialData();
    } catch (error) {
  ...
  if (loading && projects.length === 0 && users.length === 0)
    return (
      <div className="flex flex-col items-center justify-center py-40 gap-8">
        ...
        <div className="text-center space-y-2">
          <p className="text-brand-blue font-display font-medium text-xl animate-pulse tracking-[0.2em] uppercase">
            Sincronizando Core Master
          </p>
          <p className="text-gray-500 text-xs tracking-widest uppercase opacity-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-bold tracking-tight">
              {daysLeft <= 0
                ...
                : `Tienes ${daysLeft} ${daysLeft === 1 ? "día" : "días"} de acceso restante.`}
            </p>
          </div>
          <div className="text-[10px] font-black uppercase tracking-widest opacity-60">
            Protocolo IUTA/IUTAR
          </div>
        </motion.div>
      )}
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-3 px-4 py-2 bg-brand-blue/10 border border-brand-blue/20 rounded-2xl w-fit backdrop-blur-md">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-blue">
              {isAdmin
                ...
                : "NÚCLEO DE INVESTIGACIÓN ACTIVO"}
            </span>
          </div>
          ...
          <h1 className="text-5xl sm:text-7xl lg:text-8xl font-display font-bold tracking-tighter text-gray-900 leading-[0.85] filter drop-shadow-xl">
            {activeTab === "projects" ? (
              <>
                Potenciando tu <br />
                <span className="font-serif italic text-brand-blue text-glow">
                  descubrimiento.
                </span>
              </>
            ) : (
              <>
                Gestión de <br />
                <span className="font-serif italic text-brand-indigo text-glow">
                  investigadores.
                </span>
              </>
            )}
            ...
            <div className="flex bg-gray-900/5 backdrop-blur-xl p-1.5 rounded-3xl border border-gray-900/10 w-full sm:w-auto shadow-inner">
              ...
              <button
                ...
                className={cn(
                  "flex-1 sm:flex-none flex items-center justify-center gap-3 px-8 py-4 rounded-2xl font-bold text-xs uppercase tracking-widest transition-all",
                  activeTab === "users"
                    ? "bg-white text-brand-indigo shadow-xl scale-105"
                    : "text-gray-400 hover:text-gray-700",
                )}
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="glass-premium rounded-[3rem] p-10 flex flex-col justify-between shadow-premium border-white/40 hover:border-brand-blue/30 transition-all duration-700 group relative overflow-hidden">
                ...
                <div className="space-y-4 mt-8 relative z-10">
                  <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">
                    Optimización IA Core
                  </p>
                  <p className="text-5xl font-display font-bold text-gray-900 tracking-tighter">
                    98.4
                    <span className="text-lg text-brand-blue ml-1 hover:animate-pulse">
                      %
                    </span>
                  </p>
              ...
              <div className="glass-premium rounded-[3rem] p-10 flex flex-col justify-between shadow-premium border-white/40 hover:border-brand-indigo/30 transition-all duration-700 group relative overflow-hidden">
                ...
                <div className="space-y-4 mt-8 relative z-10">
                  ...
                  <p className="text-5xl font-display font-bold text-gray-900 tracking-tighter">
                    OS{" "}
                    <span className="font-serif italic text-brand-indigo">
                      v2.5
                    </span>
                  </p>
                  <div className="flex items-center gap-3 mt-4">
                    <div className="flex -space-x-2">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="w-6 h-6 rounded-lg bg-gray-100 border-2 border-white shadow-sm flex items-center justify-center text-[8px] font-black"
                        >
                          {i}
                        </div>
                      ))}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-8">
                ...
                <div className="flex bg-gray-900/5 p-1 rounded-2xl border border-gray-900/5">
                  <button
                    ...
                    className={cn(
                      "px-4 py-2 rounded-xl transition-all flex items-center gap-2 font-bold text-[10px]",
                      viewMode === "grid"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
                {filteredProjects.length > 0 ? (
                        ...
                        <div className="h-full flex flex-col justify-between gap-10">
                          <div className="space-y-6">
                            ...
                            <p className="text-gray-400 text-sm line-clamp-2 font-medium leading-relaxed opacity-70">
                              {p.hypothesis ||
                                "Base de investigación preliminar sin hipótesis definida."}
                            </p>
                          </div>
                  ...
                  <div className="col-span-full py-32 text-center border-2 border-dashed border-gray-100 rounded-[4rem]">
                    ...
                    <h3 className="text-3xl font-serif italic text-gray-900 mb-2">
                      Sin hallazgos
                    </h3>
                    <p className="text-gray-400 font-light italic">
                      Refina tu búsqueda o crea una nueva investigación.
                    </p>
                  </div>
                )}
              </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-50/50 border-b border-gray-50">
                      <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                        Investigación
                      </th>
                      <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                        Institución
                    ...
                    {filteredProjects.map((p) => (
                      ...
                      >
                        <td className="px-10 py-8">
                          ...
                          <p className="text-[10px] font-black tracking-widest text-gray-300 uppercase mt-1">
                            ID: {p.id.slice(0, 8)}
                          </p>
                        </td>
                        <td className="px-10 py-8 text-gray-500 font-bold tracking-widest text-xs uppercase">
                          {p.university}
                        </td>
                        <td className="px-10 py-8">
                          <span
          ...
          >
              ...
              <div className="bg-gray-900 rounded-[3rem] p-10 text-white relative overflow-hidden group shadow-2xl shadow-brand-indigo/20 border border-white/5">
                ...
                <div className="relative z-10 space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-brand-indigo font-bold tracking-widest mt-4 uppercase opacity-60">
                      Nodos Registrados
                    </p>
                  </div>
                </div>
              ...
              <div className="md:col-span-3 glass-premium rounded-[3rem] p-10 flex flex-col sm:flex-row items-center justify-between gap-10 shadow-premium border-white/40 relative overflow-hidden group">
                ...
                <div className="flex-1 space-y-6 relative z-10 text-center sm:text-left">
                  ...
                  <p className="text-gray-500 text-sm leading-relaxed max-w-lg font-medium opacity-70">
                    Monitoreo algorítmico y control de accesos centralizado bajo
                    el estándar de seguridad científico v2.5.
                  </p>
                </div>
                ...
                <div className="text-center sm:pr-10 relative z-10">
                  ...
                  <p className="text-6xl font-display font-bold text-brand-blue tracking-tighter">
                    {
                      users.filter(
                        (u) => !u.expiresAt || Date.now() < u.expiresAt,
                      ).length
                    }
                  </p>
                  <div className="w-full h-1 bg-gray-100 rounded-full overflow-hidden mt-4">
                    <div
                      ...
                      style={{
                        width: `${(users.filter((u) => !u.expiresAt || Date.now() < u.expiresAt).length / (users.length || 1)) * 100}%`,
                      }}
                    />
                  </div>
                </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pt-16 border-t border-gray-900/5">
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-300">
                  Terminal de Bio-Acceso Scientific OS
                </p>
              </div>
              <div className="relative w-full md:w-96 group">
                        ...
                        >
                          <td className="px-10 py-10">
                            <div className="flex items-center gap-6">
                              ...
                              <div className="space-y-1">
                                <p className="text-xl font-display font-bold text-gray-900 tracking-tight">
                          ...
                          <td className="px-10 py-10">
                            ...
                            <p className="text-[8px] text-gray-200 mt-1 uppercase font-bold">
                              V-SYNC-OS
                            </p>
                          </td>
                            ...
                            <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0 transition-all duration-300">
                              <button
                                onClick={() => {
                                  ...
                                  setEditRole(u.role || "researcher");
                                  setEditStatus(u.status || "active");
                                  setEditPassword(u.password || "123456");
                                  setEditDuration(-1);
                                }}
                                className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-100 border border-transparent rounded-xl transition-all"
                              ...
                              <button
                                ...
                                className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 hover:border-red-100 border border-transparent rounded-xl transition-all"
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm mt-1 italic font-medium">
                        Diseña el futuro de tu investigación científica.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => setIsModalOpen(false)}
                    className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-400 hover:text-gray-900 transition-all hover:bg-gray-100"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                ...
                <form onSubmit={handleCreateProject} className="space-y-10">
                  <div className="space-y-8">
                    <div className="space-y-4">
                      <label className="text-[9px] font-black text-gray-400 uppercase tracking-[0.3em] pl-4">
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] pl-4">
                        Idea Central / Hipótesis
                      </label>
                      <textarea
                        required
                        rows={2}
                      ...
                      <div className="space-y-3">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setNewProject({
                              ...newProject,
                              university: e.target.value as any,
                            })
                          }
                        >
                          <option value="IUTA">
                            IUTA (Instituto Universitario de Tecnología de
                            Administración)
                          </option>
                          <option value="IUTAR">
                            IUTAR (I.U. de Tecnología Rodolfo Loero Arismendi)
                          </option>
                          <option value="UPTAEB">
                            UPTAEB (U.P. Territorial Andrés Eloy Blanco)
                          </option>
                        </select>
                      </div>
                      <div className="flex items-end">
                        <div className="w-full h-full p-6 bg-blue-50 rounded-[2rem] border border-blue-100 flex items-center justify-center gap-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-blue-400 mt-1 font-bold">
                              Protocolo validado v2024
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  >
                    ...
                    {isCreating
                      ? "Estableciendo parámetros base..."
                      : "INICIAR REDACCIÓN ACADÉMICA"}
                  </button>
                </form>
              </div>
            </motion.div>
              ...
              <div className="p-16">
                <div className="flex items-center justify-between mb-12">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h2 className="text-4xl font-serif font-bold text-gray-900 italic tracking-tight">
                        Habilitar Acceso
                      </h2>
                      <p className="text-gray-400 text-sm mt-1 italic">
                        Otorga credenciales de nivel investigativo.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => setIsUserModalOpen(false)}
                    className="w-12 h-12 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:text-gray-900 transition-all hover:bg-gray-100"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-start gap-2 pl-4">
                        ...
                        <p className="text-[10px] text-gray-400 italic font-medium">
                          El usuario debe solicitar su UID desde su perfil antes
                          de ser habilitado.
                        </p>
                      </div>
                    </div>
        ...
        {editingUser && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-2xl relative overflow-hidden flex flex-col"
            >
              <div className="h-2 bg-gradient-to-r from-brand-indigo via-blue-600 to-brand-indigo" />
              <div className="p-16">
                <div className="flex items-center justify-between mb-12">
                  <div className="flex items-center gap-6">
                    <div className="w-16 h-16 bg-brand-indigo rounded-3xl flex items-center justify-center text-white shadow-2xl shadow-indigo-500/30 transform -rotate-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm mt-1 italic font-medium">
                        Modifica vigencia y privilegios del investigador.
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <form onSubmit={handleSaveEditUser} className="space-y-10">
                  <div className="space-y-8">
                    {/* Unique ID & Email (Readonly) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] pl-4">
                          UID del Sistema
                        ...
                        <input
                          type="text"
                          readOnly
                          className="w-full px-8 py-6 bg-gray-50 border border-gray-100 rounded-[2rem] text-xs font-mono font-bold text-gray-400 outline-none cursor-not-allowed select-all"
                      ...
                      <div className="space-y-3">
                        ...
                        <input
                          type="text"
                          readOnly
                          className="w-full px-8 py-6 bg-gray-50 border border-gray-100 rounded-[2rem] text-xs font-semibold text-gray-400 outline-none cursor-not-allowed"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        ...
                        <input
                          required
                          type="text"
                          className="w-full px-8 py-6 bg-gray-50 border border-gray-100 focus:bg-white focus:border-indigo-200 rounded-[2rem] outline-none transition-all text-sm font-semibold"
                          value={editName}
                      ...
                      <div className="space-y-3">
                        ...
                        <input
                          required
                          type="text"
                          className="w-full px-8 py-6 bg-gray-50 border border-gray-100 focus:bg-white focus:border-indigo-200 rounded-[2rem] outline-none transition-all text-sm font-mono font-semibold text-indigo-600"
                          value={editPassword}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="space-y-3">
                        ...
                        <select
                          className="w-full px-8 py-6 bg-gray-50 border border-gray-100 focus:bg-white focus:border-indigo-200 rounded-[2rem] outline-none transition-all appearance-none font-bold text-gray-600 text-xs cursor-pointer shadow-sm"
                          value={editDuration}
                          onChange={(e) => setEditDuration(Number(e.target.value))}
                        >
                          {EDIT_EXPIRATION_OPTIONS.map((opt) => (
                            <option key={opt.value} value={opt.value}>
                              {opt.label}
                            </option>
                          ))}
